...
Usage:
    python -m rag.ingest upload FILE [FILE ...]   Upload images to S3
    python -m rag.ingest index                     Sync S3 → local → Qdrant
    python -m rag.ingest stats                     Show indexing progress
"""
...
def main() -> None:
    ...
    if command == "upload":
        if len(sys.argv) < 3:
            print("Usage: python -m rag.ingest upload FILE [FILE ...]")
            sys.exit(1)
        _upload(sys.argv[2:])
    elif command == "index":
        _index()
